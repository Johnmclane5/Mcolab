...
async def generate_gif_thumbnail(video_file, duration):
    ...
    if duration < 25:
        ...
        cmd = [
            ...
            video_file,
            "-vf",
            "fps=8,scale=380:-1:flags=lanczos,split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse",
            "-loop",
            "0",
            output,
        ]
        ...
        cmd = [
            ...
            video_file,
            "-filter_complex",
            "[0:v]fps=8,scale=380:-1:flags=lanczos[v0];"
            "[1:v]fps=8,scale=380:-1:flags=lanczos[v1];"
            "[2:v]fps=8,scale=380:-1:flags=lanczos[v2];"
